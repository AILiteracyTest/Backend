import sys, os, time, glob, pdb, cv2
import numpy as np
import torch
...
from datasets_ae import DAE_dataset
...
def plot_losses(running_train_loss, running_val_loss,
                train_epoch_loss, val_epoch_loss, epoch):
    ...
    plt.savefig(os.path.join(
        losses_dir, 'losses_{}.png'.format(str(epoch + 1).zfill(2)))
    )
    plt.close(fig)


# -----------------------------
# Transform 정의 (RGB, 256x256, [-1,1])
# -----------------------------
...
# -----------------------------
# 모델 정의 (UNet AE: 3채널 입력/출력)
# -----------------------------
# UNet이 in_channels 인자를 지원한다면 아래처럼 사용
model = UNet(
    in_channels=3,        # RGB 입력
    n_classes=3,          # RGB 출력 (AE)
    depth=cfg.depth,
    padding=True          # keep same spatial size
).to(device)

# 만약 네 unet.py의 UNet 시그니처가 (n_classes, depth, padding, ...)처럼
# in_channels 없이 정의되어 있다면, 위를 아래처럼 바꾸고
# unet.py에서 in_channels 기본값을 3으로 맞춰줘야 함:
# model = UNet(n_classes=3, depth=cfg.depth, padding=True).to(device)
...
print('\nmodel has {:.3f} M parameters'.format(count_parameters(model)))
print(f'\nloss_fn        : {loss_fn}')
print(f'lr             : {lr}')
...
for epoch in range(epochs_till_now, epochs_till_now + epochs):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_loader):
        ...
        targets = targets.to(device)   # AE니까 inputs == targets 이지만, 인터페이스상 분리
        ...
        if (batch_idx + 1) % log_interval == 0:
            ...
            print(
                'train loss @batch_idx {}/{}: {:.6f} in {} mins {:.2f} secs (per batch)'.format(
                    str(batch_idx + 1).zfill(len(str(len(train_loader)))),
                    len(train_loader),
                    loss.item(),
                    int(m),
                    s
    ...
    h, m = divmod(m, 60)
    print('\nepoch train time: {} hrs {} mins {} secs'.format(
        int(h), int(m), int(s)))
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(val_loader):
            ...
            if (batch_idx + 1) % log_interval == 0:
                print(
                    'val loss   @batch_idx {}/{}: {:.6f}'.format(
                        str(batch_idx + 1).zfill(len(str(len(val_loader)))),
                        len(val_loader),
                        loss.item()
                    )
                )
    ...
    h, m = divmod(m, 60)
    print('\nepoch val   time: {} hrs {} mins {} secs'.format(
        int(h), int(m), int(s)))
